          ...
          {REGIONS.map(region => {
            ...
            return (
              <div key={region} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div className="bg-[#4a6478] px-6 py-4 flex items-center justify-between">
                  <h2 className="text-white font-semibold text-base tracking-wide">{region}</h2>
                  {regionToken && (
                    <a
                      href={`/r/${regionToken}`}
                      target="_blank"
                      className="text-xs bg-[#7ab82a] text-white px-3 py-1.5 rounded-lg font-medium hover:bg-[#6aa020] transition-colors"
                    >
                      Ver link región ↗
                    </a>
                  )}
