...
export default function ClientPage() {
  ...
  async function loadProject() {
    const { data } = await supabase.from('projects').select('*').eq('client_token', token).single()
    ...
    <div className="min-h-screen bg-[#1e2a36] flex flex-col">
      ...
      <header className="bg-[#15202b] shadow-md px-8 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-xs">{project?.region}</p>
        </div>
      </header>
        ...
        {selectedImage && (
          <div className="w-80 bg-[#15202b] border-l border-slate-700 p-4 overflow-y-auto">
            ...
            <div className="mt-4 space-y-2">
              <input value={author} onChange={e => setAuthor(e.target.value)} placeholder="Tu nombre" className="w-full bg-slate-700 text-white text-sm px-3 py-2.5 rounded-xl border border-slate-600 focus:outline-none focus:border-[#7ab82a] placeholder-slate-400" />
              <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Escribe un comentario..." rows={3} className="w-full bg-slate-700 text-white text-sm px-3 py-2.5 rounded-xl border border-slate-600 focus:outline-none focus:border-[#7ab82a] placeholder-slate-400 resize-none" />
