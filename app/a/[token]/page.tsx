...
export default function AdminPage() {
  ...
  async function togglePublished() {
    ...
    const val = !selectedImage.published
    await supabase.from('images').update({ published: val }).eq('id', selectedImage.id)
    const updated = { ...selectedImage, published: val }
    setSelectedImage(updated)
    setImages(images.map(i => i.id === selectedImage.id ? updated : i))
  }

  async function toggleOnInstagram(imageId: string, current: boolean, e: React.MouseEvent) {
    e.stopPropagation()
    const val = !current
    await supabase.from('images').update({ on_instagram: val }).eq('id', imageId)
    setImages(prev => prev.map(i => i.id === imageId ? { ...i, on_instagram: val } : i))
    if (selectedImage?.id === imageId) setSelectedImage(prev => prev ? { ...prev, on_instagram: val } : null)
  }

  async function togglePublishedById(imageId: string, current: boolean, e: React.MouseEvent) {
    e.stopPropagation()
    const val = !current
    await supabase.from('images').update({ published: val }).eq('id', imageId)
    const updatedImages = images.map(i => i.id === imageId ? { ...i, published: val } : i)
    setImages(updatedImages)
    if (selectedImage?.id === imageId) setSelectedImage(prev => prev ? { ...prev, published: val } : null)

    // Feature 3: auto-archive when all images are published
    if (val && project) {
      const { data: allDeliveries } = await supabase.from('deliveries').select('id').eq('project_id', project.id)
      if (allDeliveries?.length) {
        const deliveryIds = allDeliveries.map(d => d.id)
        const { data: allImgs } = await supabase.from('images').select('id, published').in('delivery_id', deliveryIds)
        if (allImgs?.length && allImgs.every(i => i.published)) {
          const archived_at = new Date().toISOString()
          await supabase.from('projects').update({ archived: true, archived_at }).eq('id', project.id)
          setProject(prev => prev ? { ...prev, archived: true, archived_at } : null)
  ...
  return (
    <div className="min-h-screen bg-[#1e2a36] flex flex-col">
      {archivedNotice && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-gradient-to-r from-purple-600 to-pink-500 text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 text-sm font-semibold">
          ...
          ¡Campaña archivada! Todas las imágenes están en Instagram.
          <button onClick={() => setArchivedNotice(false)} className="ml-2 opacity-70 hover:opacity-100">✕</button>
        </div>
      )}
                ...
                {images.map((img, idx) => (
                  ...
                  >
                    <div className="relative h-36 md:h-48 bg-slate-700">
                      ...
                      <button
                        onClick={e => togglePublishedById(img.id, img.published, e)}
                        title={img.published ? 'Quitar de publicado' : 'Marcar como publicado'}
                        className={`absolute top-2 right-2 rounded-full w-6 h-6 flex items-center justify-center shadow transition-all ${img.published ? 'bg-green-500 hover:opacity-70' : 'bg-slate-600/60 opacity-0 group-hover:opacity-100 hover:bg-green-500'}`}
                      >
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="white"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
                      </button>
                      <button
                        onClick={e => toggleOnInstagram(img.id, img.on_instagram, e)}
                        title={img.on_instagram ? 'Quitar de Instagram' : 'Marcar como subido a Instagram'}
                        className={`absolute top-10 right-2 rounded-full w-6 h-6 flex items-center justify-center shadow transition-all ${img.on_instagram ? 'bg-gradient-to-tr from-purple-600 to-pink-500 hover:opacity-70' : 'bg-slate-600/60 opacity-0 group-hover:opacity-100 hover:bg-gradient-to-tr hover:from-purple-600 hover:to-pink-500'}`}
                      >
